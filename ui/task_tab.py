...
class DubRow(QFrame):
    ...
    def __init__(self, path: str, api: Api, preselect: list[int] | None = None) -> None:
        ...
        self.ref_tag.setObjectName("tag")
...
class TaskTab(QWidget):
    ...
    def _build(self) -> None:
        ...
        self.ref_combo = tame_combo(QComboBox(), max_width=460, chars=24)
        ...
        self.g_head.setObjectName("gheader")
        self.g_head.setCursor(Qt.PointingHandCursor)
        ...
        self.tmp_edit.setPlaceholderText(tr("set.keep_tmp_dir"))
        self.tmp_edit.setMinimumWidth(240)
        self.tmp_edit.setMaximumWidth(460)
        self.tmp_edit.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        trow.addStretch(1)
        self.tmp_row = QWidget()               # строка каталога целиком скрыта, пока опция выкл:
        self.tmp_row.setLayout(trow)           # серые «мёртвые» поля выглядят как дефект вёрстки
        gb.addWidget(self.tmp_row)
        self.tmp_row.setVisible(self.c_tmp.isChecked())
        self.c_tmp.toggled.connect(self.tmp_row.setVisible)
